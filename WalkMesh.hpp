...
struct WalkMesh
{
	...
	glm::vec3 to_world_point(WalkPoint const &wp) const
	{
		...
		return wp.weights.x * vertices[wp.indices.x] + wp.weights.y * vertices[wp.indices.y] + wp.weights.z * vertices[wp.indices.z];
	}
	...
	glm::vec3 to_world_smooth_normal(WalkPoint const &wp) const
	{
		return glm::normalize(
			wp.weights.x * normals[wp.indices.x] + wp.weights.y * normals[wp.indices.y] + wp.weights.z * normals[wp.indices.z]);
	}
