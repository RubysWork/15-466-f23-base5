...
void WalkMesh::walk_in_triangle(WalkPoint const &start, glm::vec3 const &step, WalkPoint *end_, float *time_) const
{
	...
	// credit: stroucki's idea
	step_coords = glm::vec3(0.0f);
	// for barycentric_weights
	...
	// get the point on triangle
	auto to_world_point = [this](WalkPoint const &wp)
	{
		return wp.weights[0] * vertices[wp.indices[0]] + wp.weights[1] * vertices[wp.indices[1]] + wp.weights[2] * vertices[wp.indices[2]];
	};

	glm::vec3 wp_start = to_world_point(start);
	glm::vec3 dest = wp_start + step;
	glm::vec3 bw_dest = barycentric_weights(a, b, c, dest);
...
bool WalkMesh::cross_edge(WalkPoint const &start, WalkPoint *end_, glm::quat *rotation_) const
{
	...
	// credit: ziqi Ye's idea
	auto iterator = next_vertex.find(glm::uvec2(start.indices.y, start.indices.x));
	if (iterator != next_vertex.end())
	{
		end.indices[0] = start.indices[1];
		end.indices[1] = start.indices[0];
		end.indices[2] = iterator->second;
		end.weights[0] = start.weights[1];
		end.weights[1] = start.weights[0];
		end.weights[2] = 0.0f;

		glm::vec3 const &start_a = vertices[start.indices.x];
		glm::vec3 const &start_b = vertices[start.indices.y];
		glm::vec3 const &start_c = vertices[start.indices.z];
		glm::vec3 start_normal = glm::normalize(glm::cross(start_b - start_a, start_c - start_a));

		glm::vec3 const &end_a = vertices[end.indices.x];
		glm::vec3 const &end_b = vertices[end.indices.y];
		glm::vec3 const &end_c = vertices[end.indices.z];
...
WalkMeshes::WalkMeshes(std::string const &filename)
{
	...
	for (auto const &e : index)
	{
		...
		{
			if (!((e.vertex_begin <= triangles[ti].x && triangles[ti].x < e.vertex_end) && (e.vertex_begin <= triangles[ti].y && triangles[ti].y < e.vertex_end) && (e.vertex_begin <= triangles[ti].z && triangles[ti].z < e.vertex_end)))
			{
